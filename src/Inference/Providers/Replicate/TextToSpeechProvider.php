...
/**
 * Replicate text-to-speech provider.
 *
 * @see https://replicate.com/docs/reference/http#create-prediction
 */
class TextToSpeechProvider extends BaseReplicateProvider
{
    public function preparePayload(array $args, string $model): array
    {
        $input = [];

        // Replicate TTS models typically use 'text' instead of 'prompt'
        if (isset($args['inputs'])) {
            $input['text'] = $args['inputs'];
        }
    ...
    public function getResponse(mixed $response): mixed
    {
        ...
        throw new OutputValidationException(
            'Expected Replicate TTS output to be URL string or array of URLs',
            $response
        );
    }
}
